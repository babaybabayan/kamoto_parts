    ...
    <div class="col-md-11">
        ...
        <div class="modal-dialog modal-sm">
          <div class="modal-content">
            ...
            <form action="#" method="post" enctype="multipart/form-data" class="form-horizontal brgform" id="brgform" name="brgform">
      ...
      <thead>
        ...
        @foreach($brg as $b)
          <tr>
            <td><a href="#" data-toggle="modal" data-target=".hst{{ $b->idb }}">{{ $b->code_product }}</a></td>
            <td><a href="#" data-toggle="modal" data-target=".hst{{ $b->idb }}">{{ $b->name }}</a></td>
            <td style="text-align: center"><a href="#" data-toggle="modal" data-target=".hst{{ $b->idb }}">{{ $b->nameu }}</a></td>
            <td style="text-align: center">
              <button type="button" class="btn btn-success" data-toggle="modal" data-target=".ubah{{ $b->idb }}"><span class="fa fa-edit"></span></button>
            </td>
          </tr>
          <div class="modal fade ubah{{ $b->idb }}" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-sm">
              <div class="modal-content">
                ...
                <form action="/brg/ubah/{{ $b->idb }}" method="post" enctype="multipart/form-data" id="demo-form" data-parsley-validate>
                  <div class="modal-body">
                    {{ csrf_field() }}
                    {{ method_field('PUT') }}
                    <label for="ukode">Kode</label>
                    <input type="text" id="ukode" class="form-control" name="ukode" value="{{ $b->code_product }}" autocomplete="off">
                    <label for="unama">Nama</label>
                    <input type="text" id="unama" class="form-control" name="unama" value="{{ $b->name }}" autocomplete="off">
