      ...
      <tbody>
        ...
        @foreach($drpj as $d)
          ...
          <tr>
            ...
            <td style="text-align: center">{{ $d->qtyp }}</td>
            <td style="text-align: right">{{ rupiah($d->price) }}</td>
            <td style="text-align: center">{{ $d->disc }}</td>
            <td style="text-align: right">{{ rupiah($dis2) }}</td>
            <td style="text-align: center">{{ $d->rtr }}</td>
          ...
          <div class="modal fade ubah{{ $d->idp }}" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-sm">
              <div class="modal-content">
                ...
                <form action="/rtr/drpnj/insdrpnj" method="post" enctype="multipart/form-data" id="demo-form" data-parsley-validate>
                  <div class="modal-body">
                    {{ csrf_field() }}
                    <input type="hidden" name="idpym" value="{{ $d->id_payment }}">
                    <input type="hidden" name="idbrg" value="{{ $d->id_product }}">
                    <label for="jmlrtr">Jumlah</label>
                    <input type="text" id="jmlrtr" class="form-control" name="jmlrtr" value="{{ $d->rtr }}" autocomplete="off">
                  </div>
