...
@section('content')
    ...
    <div class="row">
        <div class="col-md-6" style="text-align: left">
            <a href="#" data-toggle="modal" data-target=".listbrgpmb">
                <h5><i><u>Klik disini untuk membuka list Barang</u></i></h5>
            </a>
        </div>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="#" method="post" enctype="multipart/form-data" class="form-horizontal mypmbform"
                            id="mypmbform" name="mypmbform">
                            {{ csrf_field() }}
                            <table class="table table-striped table-bordered dtlbtp" style="width: 875px">
                                ...
                                <tbody>
                                    @foreach ($brg as $b)
                                        <tr>
                                            <td><input type="checkbox" data-input-id="{{ $b->id }}"
                                                    onclick="insbrgpmb(this)"></td>
                                            <td>{{ $b->code_product }}</td>
                                            <td>{{ $b->name }}</td>
                                            <td style="text-align: center">
                                                {{ $sldstk = \App\Models\Barang_harga::where(['id_product' => $b->id])->pluck('quantity')->sum() }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
